...
- `2024-09-12`: [@samuelroland](https://codeberg.org/samuelroland) released the first version on [npmjs.com](https://www.npmjs.com/package/prjs) under `@delibay/prjs` (because `prjs` name conflicts with a squatting package `pr.js`). PRJS is going to be migrate to Delibay's Git organisation on codeberg.org later, so releasing it under this name makes sense. Before the release, he continued to fix a few bugs, implemented a partial list system, added a build system, tried to write some automated tests, and wrote a CHANGELOG.md.
